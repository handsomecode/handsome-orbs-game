...
(function () {
  ...
  Orbs = function (_container, config) {
    ...
    self.updateHighScore = function (difficultyLevel) {
      ...
      if (self.data.scoresList.highScore[difficultyLevel].count <= self.data.scoresList.score.count) {
        self.data.scoresList.highScore[difficultyLevel].count = self.data.scoresList.score.count;

        self.setCookie(difficultyLevel + ':' + 'high-score', self.data.scoresList.highScore[difficultyLevel].count);

        self.data.scoresList.highScore[difficultyLevel]._count.textContent = self.data.scoresList.highScore[difficultyLevel].count;
      }
    };
    ...
    self.changeMode = function (mode) {
      ...
      for (var difficultyId in self.data.scoresList.highScore) {
        self.data.scoresList.highScore[difficultyId]._element.classList.remove(self.config.classes.highScoreboards.active);
      }

      self.data.scoresList.highScore[self.settings.id]._element.classList.add(self.config.classes.highScoreboards.active); // why element is undefined???
    ...
    self.generateScoreboard = function () {
      for (var difficultyId in self.data.scoresList.highScore) {
        self.data.scoresList.highScore[difficultyId].count = self.getCookie(difficultyId + ':' + 'high-score');

        if (!self.data.scoresList.highScore[difficultyId].count) {
          self.data.scoresList.highScore[difficultyId].count = 0;
        }

        self.data.scoresList.highScore[difficultyId]._count.textContent = self.data.scoresList.highScore[difficultyId].count;
      }
    };

    self.generateModeButton = function (_highScoresList, highScore) {
      var _highScore = document.createElement('li'),
          _highScoreTitle = document.createElement('span'),
          _highScoreLabel = document.createElement('span'),
          _highScoreCounter = document.createElement('span');

      _highScore.classList.add(self.config.classes.highScoreboards.default);
      _highScore.classList.add(self.config.classes.highScoreboards.default + '--' + highScore.id);
      _highScoreTitle.classList.add(self.config.classes.highScoreboards.title);
      _highScoreTitle.textContent = highScore.name;
      _highScoreLabel.classList.add(self.config.classes.highScoreboards.label);
      _highScoreLabel.textContent = 'Highscore';
      _highScoreCounter.classList.add(self.config.classes.highScoreboards.count);
      _highScoreCounter.textContent = highScore.count;

      self.data.scoresList.highScore[highScore.id]._element = _highScore;
      self.data.scoresList.highScore[highScore.id]._count = _highScoreCounter;

      _highScore.appendChild(_highScoreTitle);
      _highScore.appendChild(_highScoreLabel);
      _highScore.appendChild(_highScoreCounter);

      _highScoresList.appendChild(_highScore);

      _highScore.addEventListener('click', function () {
        if (self.settings.id !== highScore.id) {
          self.openAlert(self.config.alertChangeDifficultyText, function () {
            self.generateBoard(highScore.id);
          });
        }
      });
    };
    ...
    self.generateSidebar = function () {
      ...
      var _highScoresList = document.createElement('ul');

      _highScoresList.classList.add(self.config.classes.highScoreboards.list);

      for (var highScoreId in self.data.scoresList.highScore) {
        var highScore = self.data.scoresList.highScore[highScoreId];
        highScore.id = highScoreId;

        self.generateModeButton(_highScoresList, highScore);
      }

      self.data._sidebar.appendChild(_highScoresList);
